...
class MyLSTM(nn.Module):
    ...
    def __init__(self, input_size, hidden_dim):
        ...
        self.fc = nn.Linear(hidden_dim, 2)  # 创建一个线性层对象，将LSTM的输出映射到二分类的结果上.hidden_dim表示输入维度,2表示输出维度（二分类）
        self.softmax = nn.Softmax(dim=1)  # 创建一个softmax层对象，用于将输出进行归一化，得到概率分布.dim=1表示对第一维进行softmax操作（即每个样本的输出）。
